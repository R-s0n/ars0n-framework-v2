...
const ARJUN_FIELDS = {
  ...
  delay: 'Delay between requests within a thread, in seconds. Combines with threads to set the aggregate rate, so it cannot express a ceiling on its own. Prefer rateLimit when you have a measured budget.',
  rateLimit: 'Hard ceiling on requests per second, mapping to arjun --rate-limit. 0 leaves it uncapped, which is arjun default of 9999. Unlike delay and threads, this states a budget directly rather than as a product of two knobs and the target latency. Set it from the safe_rps the Routing and WAF Probe measured: arjun full default wordlist runs about 15 req/s against a 53ms target, so a probe reporting safe_rps 5 is overrun threefold unless this is set.',
  timeout: 'Per-request timeout in seconds.',
...
const PROBE_FIELDS = {
  ...
  arjun: ['rateLimit', 'delay', 'threads', 'timeout', 'headers', 'method'],
