...
var ghauriOwned = map[string]string{
	...
	// The same defect commix has, and it was fixed there and not here. ghauri keys its session by
	// HOST, in /root/.ghauri/<host>/session.sqlite, so the verdict from the FIRST vector is replayed
	// for every later vector on that host. Measured: a target ghauri had already seen answered
	// "all tested parameters do not appear to be injectable" in one second, having sent nothing;
	// the same command with --flush-session found a boolean-based blind injection.
	"--flush-session": "Always set. ghauri consults a session stored per host and replays its verdict " +
		"for every later vector on that host, so without this a 53-vector scan is really one scan and " +
		"52 replays of it, all reported clean.",
}
...
var ghauriOptions = map[string]VectorOptionMeta{
	...
	// int, NOT float like sqlmap's. ghauri 1.4.3 parses this with type=int and dies with
	// "argument --delay: invalid int value: '0.5'" and exit 2 before it sends a single request. It
	// was declared float here, copied from the sqlmap table, and a whole 53 vector run came back
	// clean in forty seconds because of it.
	"delay":         {Kind: "int", Group: "Request", Label: "Delay between requests", Flag: "--delay", Placeholder: "0 seconds, whole seconds only"},
	"timeout":       {Kind: "int", Group: "Request", Label: "Timeout", Flag: "--timeout", Placeholder: "30 seconds"},
	"retries":       {Kind: "int", Group: "Request", Label: "Retries", Flag: "--retries", Placeholder: "3"},
