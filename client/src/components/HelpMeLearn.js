    ...
    'attackTools:redirect-ssrf': {
      ...
      items: [
        ...
        },
        {
          ...
          answers: [
            "They are a pipeline rather than three alternatives, and there are two detectors, not one. REcollapse is this framework's own scan: REcollapse itself only mutates a string, so the framework takes that payload list and sends it at every probeable parameter, each payload carrying a canary unique to that one input. That is what lets a callback say which parameter called out rather than only that the host did. Nuclei DAST is the second detector, running ProjectDiscovery's stock templates, and it never sees your webhook. SSRFmap is the exploitation stage and only makes sense once one of the two has found something.",
