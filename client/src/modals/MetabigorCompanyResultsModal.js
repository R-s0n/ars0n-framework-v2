...
export const MetabigorCompanyResultsModal = ({ 
  ...
  useEffect(() => {
    if (showMetabigorCompanyResultsModal && metabigorCompanyResults && metabigorCompanyResults.scan_id) {
      fetchMetabigorData(metabigorCompanyResults.scan_id);
    } else if (showMetabigorCompanyResultsModal && metabigorCompanyResults) {
      if (metabigorCompanyResults.status === 'error') {
        setError('Scan failed. Please check the scan details and try again.');
        setLoading(false);
      } else if (!metabigorCompanyResults.scan_id) {
        setError('No scan ID available for this scan.');
        setLoading(false);
      }
    }
  }, [showMetabigorCompanyResultsModal, metabigorCompanyResults]);

  const fetchMetabigorData = async (scanId) => {
    setLoading(true);
    setError(null);
    setNetworkRanges([]);
    setAsnData([]);
    ...
    try {
      const networkResponse = await fetch(
        `/api/metabigor-company/${scanId}/networks`
      );
      ...
      const asnResponse = await fetch(
        `/api/metabigor-company/${scanId}/asn`
      );
      ...
      console.error('Error fetching Metabigor data:', error);
  ...
  const getResultCount = () => {
    if (!metabigorCompanyResults || metabigorCompanyResults.status === 'error') return 0;
    return networkRanges.length + asnData.length;
  };

  const getExecutionTime = () => {
    if (!metabigorCompanyResults || !metabigorCompanyResults.execution_time) return 'N/A';
    return metabigorCompanyResults.execution_time;
  };

  const getScanStatus = () => {
    if (!metabigorCompanyResults) return 'Unknown';
    return metabigorCompanyResults.status || 'Unknown';
  };
  ...
  const renderContent = () => {
    if (metabigorCompanyResults && metabigorCompanyResults.status === 'error') {
      return (
        <div className="text-center py-5">
          ...
          <p className="text-muted mb-3">
            The Metabigor scan encountered an error and could not complete successfully.
          </p>
                ...
                <tbody>
                  {networkRanges.map((network) => (
                    <tr key={network.id}>
                      ...
                      <td>
                        <span className="badge bg-info">{network.scan_type}</span>
                      </td>
                      <td>
                        {network.country ? (
              ...
              <Table striped bordered hover variant="dark" className="mb-0">
                ...
                <tbody>
                  {asnData.map((asn, index) => (
                    <tr key={index}>
                      <td className="font-monospace">{asn.asn_number}</td>
                      <td>{asn.organization || 'N/A'}</td>
                      <td>
                        <span className="badge bg-info">{asn.scan_type}</span>
                      </td>
                      <td>
                        {asn.country ? (
                      ...
                      <td>
                        <button 
                          onClick={() => handleCopyText(asn.asn_number)}
                          className="btn btn-sm btn-outline-danger"
                          title="Copy ASN"
                        >
                          <MdCopyAll size={14} />
    ...
    >
      <Modal.Header closeButton>
        <Modal.Title className="text-danger">
          Metabigor Results - Network Ranges
          {metabigorCompanyResults && (
            <span className="text-white fs-6 ms-3">
              Company: {metabigorCompanyResults.company_name}
            </span>
          )}
        </Modal.Title>
      </Modal.Header>
  ...
  return (
    ...
    >
      <Modal.Header closeButton className="bg-dark border-danger">
        <Modal.Title className="text-danger">Metabigor Scan History - Network Discovery</Modal.Title>
      </Modal.Header>
      <Modal.Body className="bg-dark">
        {metabigorCompanyScans && metabigorCompanyScans.length > 0 ? (
          ...
          <div className="text-center py-4">
            <p className="text-muted">No Metabigor scans found.</p>
          </div>
        )}
      </Modal.Body>
    </Modal>
