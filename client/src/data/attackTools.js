  ...
  },
  {
    ...
    tools: [
      ...
      },
      {
        ...
        description: 'Fingerprints the GraphQL engine behind an endpoint and links it to its GraphQL Threat Matrix entry, which is where that implementation’s known weaknesses are documented.',
      },
    ],
  },
  {
    key: 'sensitive-leak',
    title: 'Sensitive Data Leaks & Exposed Secrets',
    tools: [
      {
        key: 'snallygaster',
        name: 'snallygaster',
        url: 'https://github.com/hannob/snallygaster',
        description: 'Checks an endpoint for the files people leave behind: .git, .svn, .env, .DS_Store, core dumps, backups, private keys, phpinfo and server-status. It scans the site root by default, which is the usual mistake, so point it at the directories the application actually lives in.',
      },
      {
        key: 'mantra',
        name: 'Mantra',
        url: 'https://github.com/brosck/mantra',
        description: 'Reads the JavaScript and HTML an endpoint serves and hunts hardcoded API keys and credentials in it. Pattern matching, so treat a hit as a lead and let TruffleHog decide whether the key is live.',
      },
      {
        key: 'trufflehog',
        name: 'TruffleHog',
        url: 'https://github.com/trufflesecurity/trufflehog',
        description: 'Finds and LIVE-VERIFIES leaked credentials across hundreds of detectors. Its detectors parse what they match before reporting, so a verified result is a key that still works rather than a string that looks like one.',
      },
    ],
  },
  {
    ...
    tools: [
      {
        ...
        description: 'Reconstructs a git repository from an exposed .git, even when directory listing is disabled. It downloads the whole object store, so a secret committed and later deleted is still recoverable.',
      },
      {
        key: 'gittools',
        name: 'GitTools',
        url: 'https://github.com/internetwache/GitTools',
        description: 'Downloads an exposed .git and rebuilds every commit into its own directory, which is the convenient shape when the job is grepping every historical state at once.',
      },
    ],
  },
  {
